...
#if NET8_0_OR_GREATER
                if (marshalToUiThread && _dispatcher != null)
                    _dispatcher.Dispatch(() => handler(ctx.Payload));
                else
                    handler(ctx.Payload);
#else
        handler(ctx.Payload);
...
#if NET8_0_OR_GREATER
                        if (_dispatcher != null) _dispatcher.Dispatch(() => ClientDisconnected?.Invoke(c));
                        else ClientDisconnected?.Invoke(c);
#else
...
#if NET8_0_OR_GREATER
                    if (_dispatcher != null) _dispatcher.Dispatch(() => ClientConnected?.Invoke(conn));
                    else ClientConnected?.Invoke(conn);
#else
